...
def distributed_data_generator(filename_pattern: str, batch_size: int, align_to_bos: bool):
    ...
    max_batch_span = 64 * batch_size if align_to_bos else batch_size # provide buffer to handle samples up to length local_batch_size. Used for 4 GPUs because batch-size was reduced
    while True:
...
parser.add_argument('-batch_per_device', type=int, default=48, help='Batch size per device')
...
class Hyperparameters:
    ...
    train_seq_len = cmd_args.batch_per_device*1024 # FlexAttention sequence length
    ...
    num_iterations = int(13125 * 48 / cmd_args.batch_per_device) # when using 3B tokens ADDED
    # num_iterations = 3500 # if number of iterations to run (doubled for 4 GPUs to match 8-GPU token count) ADDED
    #cooldown_frac = 0.45 # fraction of training spent cooling down the learning rate
    cooldown_frac = cmd_args.cooldown_frac # fraction of training spent cooling down the learning rate ADDED
    warmup_frac = cmd_args.warmup_frac # fraction of training for warmup
...
# validation schedule: start at step 2000, then multiply by 1.1 each time
next_val_step = 2000
val_factor = 1.1
for step in range(train_steps + 1):
    ...
    # --------------- VALIDATION SECTION -----------------
    do_val = last_step or (step >= next_val_step >= 0)
    if do_val:
        # stop the clock
        torch.cuda.synchronize()
        ...
        # advance next validation step if using the schedule
        if not last_step and step >= next_val_step:
            next_val_step = max(next_val_step + 1, int(math.ceil(next_val_step * val_factor)))
    ...
    # update tokens processed and log to wandb
